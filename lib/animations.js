...
// ── "Your stack today": a transaction crawls the broken stack, then collapses to Aleq ──
export function installStackFlow() {
  document.querySelectorAll(".sflow").forEach((flow) => {
    ...
    const yOf = (n) => n.offsetTop + n.offsetHeight / 2 - 4.5; // center token (9px) on node
    ...
    const HOP = 1150; // slow per-hop = the "lag"
    function run() {
      reset();
      ...
      const end = 280 + nodes.length * HOP;
      at(end, () => { token.style.opacity = "0"; flow.classList.add("collapsed"); });
      at(end + 2800, run); // hold the resolved state, then replay
    }
...
export function installMeterFill() {
  const bars = document.querySelectorAll(
    ".pp-learned-bar i, .pp-meter-track i, .pp-bar i, .pp-tb-bar i"
  );
