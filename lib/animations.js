...
// ── "Your stack today": one transaction crawls the broken stack, accruing days
//    at every handoff — then lands in Aleq as a balanced entry, posted day 0.
//    Plays ONCE and settles (a perpetual loop next to copy reads as decoration,
//    not evidence). Click the settled card to replay.
export function installStackFlow() {
  document.querySelectorAll(".sflow").forEach((flow) => {
    if (flow.dataset.sflow) return;
    flow.dataset.sflow = "1";
    const rail = flow.querySelector(".sflow-rail");
    const nodes = Array.from(flow.querySelectorAll(".sflow-node"));
    const token = flow.querySelector(".sflow-token");
    if (!rail || !nodes.length || !token) return;

    let timers = [];
    let running = false;
    const clear = () => { timers.forEach(clearTimeout); timers = []; };
    const at = (ms, fn) => timers.push(setTimeout(fn, ms));
    const yOf = (n) => n.offsetTop + n.offsetHeight / 2 - token.offsetHeight / 2;
    ...
    function reset() {
      ...
      token.style.transform = `translateY(${yOf(nodes[0])}px)`;
      void token.offsetWidth; // flush, so the first hop animates
    ...
    function settle() {
      ...
      flow.classList.add("collapsed");
      token.style.opacity = "0";
      running = false;
    }

    const HOP = 950; // slow per-hop = the "lag"
    function run() {
      running = true;
      reset();
      at(280, () => { token.style.opacity = "1"; });
      nodes.forEach((n, i) => {
        at(280 + i * HOP, () => {
          token.style.transform = `translateY(${yOf(n)}px)`;
          n.classList.add("lit");
        });
      });
      const end = 280 + nodes.length * HOP + 300;
      at(end, () => {
        token.style.opacity = "0";
        flow.classList.add("collapsed");
        running = false;
    ...
    // Once settled, the card replays on click.
    flow.addEventListener("click", () => { if (!running) run(); });
    ...
    if ("IntersectionObserver" in window) {
      const io = new IntersectionObserver((entries) => {
        for (const e of entries) {
          if (e.isIntersecting) { run(); io.unobserve(flow); }
        }
      }, { threshold: 0.35 });
      io.observe(flow);
    } else {
...
// ── Confidence/progress bars: fill on reveal instead of rendering static ──
//   pp-learned-bar (TAMi belief strength), pp-meter-track, pp-bar, pp-tb-bar
//   all render at their final width immediately today — a bar that never
//   fills reads as decoration, not evidence. This zeroes them at mount (while
//   still hidden behind the parent .reveal's opacity:0) and animates to the
//   real width the moment the card scrolls into view.
export function installMeterFill() {
  const bars = document.querySelectorAll(
    ".pp-learned-bar i, .pp-meter-track i, .pp-bar i, .pp-tb-bar i, .ibt-bar i"
  );
  ...
  bars.forEach((bar) => {
    ...
    if (!target) return;
    bar.dataset.fillTo = target;
  });

  if (prefersReduced || !("IntersectionObserver" in window)) return;

  bars.forEach((bar) => {
    if (!bar.dataset.fillTo) return;
    bar.style.transition = "none";
    bar.style.width = "0%";
  });
  ...
  const io = new IntersectionObserver(
    (entries) => {
      for (const e of entries) {
        ...
        const bar = e.target;
        void bar.offsetWidth; // flush the width:0 so the fill animates
        bar.style.transition = "width 900ms cubic-bezier(.2,.7,.3,1)";
        bar.style.width = bar.dataset.fillTo;
        io.unobserve(bar);
      }
    },
    { threshold: 0.4 }
  );
  bars.forEach((bar) => { if (bar.dataset.fillTo) io.observe(bar); });
}
